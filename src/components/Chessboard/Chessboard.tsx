 ...
 * @param playMove - Function to play a move
...
export default function Chessboard({playMove, pieces} : Props) {
  ...
  // It sets the active piece to the piece that is being grabbed
  // It also sets the grab position to the position of the piece that is being grabbed
  ...
  // It moves the piece to the position of the mouse
  // It also sets the active piece to null
  ...
  // It drops the piece to the position of the mouse
  // It also sets the active piece to null
  // It also checks if the piece is in the constraints of the chessboard
  // If the piece is not in the constraints of the chessboard, it moves the piece to the constraints
  ...
  for (let j = VERTICAL_AXIS.length - 1; j >= 0; j--) {
    ...
    for (let i = 0; i < HORIZONTAL_AXIS.length; i++) {
      // Calculate the number of the tile e.g 
      const number = j + i + 2;
      ...
      board.push(<Tile position={`${HORIZONTAL_AXIS[i]},${VERTICAL_AXIS[j]}`} key={`${j},${i}`} image={image} number={number} highlight={highlight} />);
    }
  }
